...
def _call_deepseek(raw_text: str, api_key: str) -> dict[str, Any]:
    ...
    prompt = {
        ...
        "content": (
            ...
            "如果用户明确给出因子数量、因子名称或公式，则优先完整输出用户指定的所有因子。\n"
            "安全表达式只能使用字段 open/high/low/close/volume/amount/returns，数字常量，四则运算，"
            "以及函数 delay, delta, mean, sum, std, max, min, correlation, rank, zscore, abs, log, kdj_j。"
            "也允许时间序列别名 ts_mean, ts_sum, ts_std, ts_corr。\n"
            "函数示例：delay(close, 5), ts_sum(returns, 5), mean(amount, 20), ts_corr(close, volume, 10), kdj_j(10)。\n"
            "如果用户定义 alpha_001 等中间因子，generated_expressions 可以使用这些名称作为 factor_name；"
            "但 expression 里不能引用 alpha_001 这类变量。composite_alpha 必须展开成基础字段和安全函数的完整表达式。\n"
